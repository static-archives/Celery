...
do
	...
	local function FetchAPI()
		...
		if not ok then
			warn("[saveinstance] Unable to find API Dump " .. version() .. ", trying latest")
			warn(err)
			API_Dump = game:HttpGet(
	...
	if ok then
		...
		warn("[saveinstance] Failed to load the API Dump")
		warn(result)
		return
	end
end
...
--- @field AlternativeWritefile boolean -- * **RISKY:** Uses breaks down file content string into segments and writes them using appendfile. This might help with crashes when it starts writing to file. ___Default:___ false (except Celery)
...
local function synsaveinstance(CustomOptions, CustomOptions2)
	...
	local OPTIONS = {
		...
		IsolateLocalPlayer = #service.StarterGui:GetChildren() == 0,
		...
		AlternativeWritefile = EXECUTOR_NAME == "Celery", -- TODO Temporary true (once removed, remove Note from docs too)
	...
	do
		-- ? Fixes crashes on Mobile Devices
		...
		if Path then
			...
			for _, className in Exclude do
				local dest = DecompileIgnore[className]
				if dest == nil then
					DecompileIgnore[className] = Exclude
				elseif type(dest) == "table" then
					for _, name in Exclude[className] do
					...
					for _, Property in proplist do
						...
						if raw == "__BREAK" then -- ! Assuming __BREAK is always returned when there's a failure to read a property
							...
							if ok then
								...
								local PropertyFix
								do
									local Inheritors = NotScriptableFixes -- ! Assuming NotScriptableFixes is defined
									repeat
										local fix
										for superclass, fixes in next, Inheritors do -- ? Celery fix (temp)
									...
									if DecompileIgnoring then -- ? Should this really prevent extraction of the original source if present ?
										...
										if hasLinkedSource then
											...
											if LinkedSource then
												...
												warn(
													"Unable to extract original source for " .. tostring(instance:GetFullName()), "Linked source:",
													LinkedSource_Url
												)
											end
										end
										...
										if should_decompile then
											...
											then
												value = "-- Server Scripts can NOT be saved" --TODO: Could be not just server scripts in the future
											else
						...
						if tag then
							...
							warn("[saveinstance] Unsupported type: ", ValueType, ClassName, PropertyName)
						end
					end
				end
	...
	local function save_game()
		...
		do
			...
			if OPTIONS.AlternativeWritefile and appendfile then
				...
				for i = 1, Length do
					local savestr = string.sub(totalstr, (i - 1) * SegmentLength + 1, i * SegmentLength)

					run_with_loading(
						"Writing to File " .. math.round(i / Length * 100) .. "% (Depends on Exec)",
						nil,
						true,
						appendfile,
						placename,
		...
		if StatusText then
			task.spawn(function()
				...
				if ok then
					...
					warn("[saveinstance] An error occurred:", err)
