...
function Drawing.Font.new(FontName, FontData)
    ...
    if string.sub(FontData, 1, 11) == "rbxasset://" then
        ...
        if not genv.isfile(FontData) then
            genv.writefile(`DrawingFontCache/{FontName}.ttf`, genv.crypt.base64.decode(FontData))
            FontData = `DrawingFontCache/{FontName}.ttf`
        end

        genv.writefile(TempPath, HttpService:JSONEncode({
            ["name"] = FontName,
            ["faces"] = {
                {
                    ["name"] = "Regular",
                    ["weight"] = 100,
                    ["style"] = "normal",
                    ["assetId"] = genv.getcustomasset(FontData)
                }
            }
        }))

        FontObject = Font.new(genv.getcustomasset(TempPath), Enum.FontWeight.Regular, Enum.FontStyle.Normal)

        genv.delfile(TempPath)
        ]]
    end
...
function Image:__SET_IMAGE(data, isUri)
    task.spawn(function()
        ...
        if not Drawing.__IMAGE_CACHE[data] then
            ...
            genv.writefile(TempPath, data)
            Drawing.__IMAGE_CACHE[data] = genv.getcustomasset(TempPath)
            genv.delfile(TempPath)
        end
...
if not genv.isfolder("DrawingFontCache") then
    genv.makefolder("DrawingFontCache")
end
...
genv["Drawing"]["Fonts"] = Drawing.Font.Fonts
genv["isrenderobj"] = function(a1)
    if type(getmetatable(a1)) == "table" then
        return getmetatable(a1).__iter and tostring(a1) == "Drawing"
    end
    return false
    --return genv.Drawing.__OBJECT_CACHE[a1] and rawget(genv.Drawing.__OBJECT_CACHE[a1], "__OBJECT_EXISTS") ~= nil
end
genv["cleardrawcache"] = Drawing.ClearCache
